...
(function () {
  ...
  function checkScroll(showSlider, scrollHandler) {
    const specsElement = document.querySelector('#Feature\\ Header') || document.querySelector('#Specs') || document.querySelector('#Specs\\ Header');
  ...
  function closeSlider() {
    Kameleoon.API.Utils.addEventListener(document, 'click', ({
      ...
      const isOverLay = target.closest('.t3-slider-overlay') && target.closest('.t3-slider-container') === null;
  ...
  function kamKgmt3ProcessGoal(goalName) {
  ...
  function showSlider() {
    if (sessionStorage.getItem('t3ModalShown') === null) {
      ...
      const pathKey = window.location.pathname.toLowerCase().replace(/\/$/, '').replace(/\.html$/, '');
      const modelMapping = {
        '/models/torres': {
      ...
      const helpText = isMobile ? `HAVE A QUESTION ABOUT THE KGM ${displayModelName}?` : `WE'RE HERE TO HELP. HAVE A QUESTION ABOUT THE KGM ${displayModelName}?`;
      const heroImg = modelConfig && modelConfig.hero || '//cdn.optimizely.com/img/15841360337/49f337a165b447fdb4e0c25b3c4ce40c.png';
      const buildImg = isMobile && modelConfig && modelConfig.Mob_buildandprice || modelConfig && modelConfig.build || buildIcon;
      const testImg = isMobile && modelConfig && modelConfig.Mob_booktestdrive || modelConfig && modelConfig.test || testIcon;
