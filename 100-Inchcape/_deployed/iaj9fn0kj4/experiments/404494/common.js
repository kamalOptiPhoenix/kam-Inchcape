...
(function () {
  ...
  (function kamKgmt1Common() {
    ...
    function validateForm() {
      const allPopupForms = Array.from(document.querySelectorAll('#popUpForm')).filter(el => !Array.from(el.classList).some(cls => cls.toLowerCase().startsWith('kgm') && cls !== 'KGMT1-form'));
    ...
    function watchPopupVisibility(popupEl) {
      if (!popupEl || Array.from(popupEl.classList).some(cls => cls.toLowerCase().startsWith('kgm'))) return;
