...
(function () {
  ...
  const config = {
    modules: {
      fetchPopup: () => fetch('https://ldv.co.nz/').then(res => res.text()).then(htmlStr => {
      ...
      addPopup: () => {
        config.modules.fetchPopup().then(popup => {
      ...
      triggerEvents: () => {
        ...
        if (window.innerWidth < 768) {
          ...
          Kameleoon.API.Utils.addEventListener(document.querySelector('body'), 'mouseleave', viewModal);
        }
      }
    }
  };
