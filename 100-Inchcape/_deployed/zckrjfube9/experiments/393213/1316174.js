...
(function () {
  ...
  function kamSubt138InsertMarkup(searchBarContent) {
    if (document.querySelector(kamSubt138Config.selectors.desktopGrid) || document.querySelector(kamSubt138Config.selectors.mobileCarousel)) {
      return;
    }
    searchBarContent.insertAdjacentHTML('afterend', kamSubt138Config.html.gridMarkup);
  }
  function kamSubt138InitCarousel() {
    const mobileTrack = document.querySelector(kamSubt138Config.selectors.mobileTrack);
    const mobileDots = document.querySelectorAll(kamSubt138Config.selectors.mobileDots);
    if (!mobileTrack || !mobileDots.length) {
      return;
    }
    const kamSubt138UpdateDots = () => {
      const activeIndex = Math.round(mobileTrack.scrollLeft / mobileTrack.clientWidth);
  ...
  function kamSubt138Init([searchBarContent]) {
    console.log('%c *** SUBT138 start ***', 'background: red;');
