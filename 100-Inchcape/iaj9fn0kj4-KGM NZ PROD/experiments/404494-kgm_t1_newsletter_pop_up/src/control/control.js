...
(function kamKgmt1Control() {
    ...
    Kameleoon.API.Core.runWhenConditionTrue(
        () => document.querySelectorAll('#popUpForm').length > 0,
        () => {
            const originalPopupForms = Array.from(document.querySelectorAll('#popUpForm')).filter(
                el => !Array.from(el.classList).some(cls => cls.toLowerCase().startsWith('kgmt'))
            );
