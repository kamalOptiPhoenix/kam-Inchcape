...
const PCAT71_FUEL_ADR_DISCLAIMER = 'The published consumption (l/100km) and emissions figures are determined '
    + 'in accordance to ADR 81/02. Actual figures will vary as they are dependent on many factors, '
    + 'including without limitation: individual driving style, road and traffic conditions, '
    + 'environmental conditions and fuel quality. Extra features and accessories '
    + '(eg: equipment, wheels or tyres used etc.) can change relevant vehicle parameters such as weight, '
    + 'rolling resistance and aerodynamics and can also affect the consumption, emission, '
    + 'and performance values of a vehicle. Generally, consumption and emissions will vary from the '
    + 'published figures in real world Australian driving conditions.';

const PCAT71_WIRELESS_DISCLAIMER = 'Compatible Apple® or AndroidTM device required';

const PCAT71_WARRANTY_DISCLAIMER = '5 Year Unlimited Kilometre Warranty on PEUGEOT new vehicles. '
    + 'Subject to terms, conditions, and exclusions. Visit ';

const PCAT71_WARRANTY_URL = 'https://www.peugeot.com.au/owners/warranty.html';

const PCAT71_ROADSIDE_DISCLAIMER = '5 Year Roadside Assist on PEUGEOT new vehicles. '
    + 'Subject to terms, conditions, and exclusions. Visit ';

const PCAT71_ROADSIDE_URL = 'https://www.peugeot.com.au/owners/peugeot-roadside-assistance.html';

const PCAT71_BATTERY_WLTP_DISCLAIMER = 'Figure based on WLTP (Worldwide Harmonised Light Vehicle Test Procedure) '
    + 'static laboratory combined average city and highway cycle test, which measure, energy consumption, range and '
    + 'emissions in passenger vehicles, figures stated for the purposes of comparison amongst vehicles tested to '
    + 'the same technical procedures only. Actual real world range results will vary depending on a combination of '
    + 'factors including but not limited to load, driving style, type of journey, vehicle configuration, battery age '
    + 'and condition, use of vehicle features (such as heating and air conditioning), as well as operating, '
    + 'environmental and climate conditions.';

const PCAT71_BATTERY_WARRANTY_DISCLAIMER = '8 years or 160,000km whichever comes sooner, from the vehicle\'s '
    + 'warranty start date. Subject to terms and conditions, visit ';


const config = {
    buildInfoTipHtml({
        message, ariaLabel, instanceId = '0', visitHref
    }) {
        const safeMessage = escapeHtml(message);
        const visitLink = visitHref
            ? `<a href="${escapeHtmlAttr(visitHref)}" class="pcat71-infotip__link" target="_blank" rel="noopener noreferrer">${escapeHtml(visitHref)}</a>`
            : '';
        const bubbleInner = safeMessage + visitLink;
        const safeAria = escapeHtml(ariaLabel);
        const tipId = `pcat71-infotip-bubble-${instanceId}`;
        const openSpan = '<span class="pcat71-infotip" tabindex="0" role="button" '
            + `aria-label="${safeAria}" aria-describedby="${tipId}">`;
        const svgOpen = '<svg class="pcat71-infotip__icon" width="14" height="14" '
            + 'viewBox="0 0 14 14" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">';
        const svgShapes = '<circle class="pcat71-infotip__ring" cx="7" cy="7" r="6"/>'
            + '<circle class="pcat71-infotip__i-part" cx="7" cy="4.25" r="0.9"/>'
            + '<rect class="pcat71-infotip__i-part" x="6.15" y="5.85" width="1.7" height="4.15" rx="0.45"/>';

        return `${openSpan + svgOpen + svgShapes}</svg>`
            + `<span class="pcat71-infotip__bubble" role="tooltip" id="${tipId}">${bubbleInner}</span>`
            + '</span>';
    },
    ...
    getCardHTML: (modelSlug) => {
        ...
        const cardsMarkup = Object.values(modelData).map((card, index) => {
            const imageBlock = card.imageDisclaimerOverlay
                ? `<div class="pcat71-card-image"><img src="${card.image}" alt="${card.title}" /></div>`
    ...
    updateIndicator(container, indicator, totalCards) {
        ...
        const index = Math.min(
            totalCards,
            Math.max(1, Math.round(container.scrollLeft / scrollStep) + 1)
        );
...
config.modelspecificdata = {
    '2008-hybrid-suv': {
        card1: {
            ...
            description:
                `Drive further on a single tank with fuel consumption from <span class="pcat71-infotip-keep"><strong>4.4L/100km.</strong>${config.buildInfoTipHtml({
        ...
        card2: {
            ...
            description:
                `Immersive curved HD display with Apple CarPlay® and Android Auto™ <span class="pcat71-infotip-keep">wireless connectivity.${config.buildInfoTipHtml({
        ...
        card4: {
            ...
            description:
                `5-year UNLIMITED KM <span class="pcat71-infotip-keep">WARRANTY${config.buildInfoTipHtml({
    ...
    '3008-suv': {
        card1: {
            ...
            description:
                `Drive further with up to 1,036km on a single tank. Fuel consumption from <span class="pcat71-infotip-keep"><strong>4.9L/100km.</strong>${config.buildInfoTipHtml({
                    ariaLabel: 'Fuel consumption disclaimer',
                    message: PCAT71_FUEL_ADR_DISCLAIMER,
    ...
    '5008-hybrid-suv': {
        card1: {
            ...
            description:
                `Drive further with up to 1,152km range & fuel consumption from <span class="pcat71-infotip-keep"><strong>5.1L/100km.</strong>${config.buildInfoTipHtml({
                    ariaLabel: 'Fuel consumption disclaimer',
                    message: PCAT71_FUEL_ADR_DISCLAIMER,
    ...
    '308-hybrid': {
        card1: {
            title: 'Turbo Hybrid Powertrain',
            description:
                `Drive further with fuel consumption from <span class="pcat71-infotip-keep"><strong>4.2L/100km.</strong>${config.buildInfoTipHtml({
                    ariaLabel: 'Fuel consumption disclaimer',
                    message: PCAT71_FUEL_ADR_DISCLAIMER,
                    instanceId: 'fuel-308'
                })}</span>`,
        ...
        card2: {
            ...
            description:
                `10” HD display with Apple CarPlay® and Android Auto™  <span class="pcat71-infotip-keep">wireless connectivity.${config.buildInfoTipHtml({
                    ariaLabel: 'Wireless connectivity disclaimer',
                    message: PCAT71_WIRELESS_DISCLAIMER,
    ...
    '408-hybrid': {
        card1: {
            title: 'Turbo Hybrid Powertrain',
            description:
                `Drive further on a single tank with fuel consumption from <span class="pcat71-infotip-keep"><strong>4.7L/100km.</strong>${config.buildInfoTipHtml({
                    ariaLabel: 'Fuel consumption disclaimer',
                    message: PCAT71_FUEL_ADR_DISCLAIMER,
                    instanceId: 'fuel-408'
                })}</span>`,
    ...
    'partner-van': {
        card1: {
            ...
            description:
                `Drive further with fuel consumption from <span class="pcat71-infotip-keep"><strong>6.3L/100km</strong>${config.buildInfoTipHtml({
                    ariaLabel: 'Fuel consumption disclaimer',
                    message: PCAT71_FUEL_ADR_DISCLAIMER,
        ...
        card2: {
            title: 'Touchscreen',
            description:
                `10” HD touchscreen display with Apple CarPlay® and Android Auto™ <span class="pcat71-infotip-keep">wireless connectivity.${config.buildInfoTipHtml({
                    ariaLabel: 'Wireless connectivity disclaimer',
                    message: PCAT71_WIRELESS_DISCLAIMER,
        ...
        card4: {
            title: '5-Years Peace of Mind',
            description:
                `5-year/200,000 KM warranty <span class="pcat71-infotip-keep">WARRANTY${config.buildInfoTipHtml({
                    ariaLabel: 'Warranty disclaimer',
                    message: PCAT71_WARRANTY_DISCLAIMER,
                    visitHref: PCAT71_WARRANTY_URL,
    ...
    'new-e-partner-van': {
        card1: {
            ...
            description:
                `100% electric driving range up to 258km with a 100kW motor and 50kWh battery. <span class="pcat71-infotip-keep">${config.buildInfoTipHtml({
        ...
        card4: {
            title: '5-Years Peace of Mind',
            description:
                `5-year/200,000 KM <span class="pcat71-infotip-keep">warranty${config.buildInfoTipHtml({
                    ariaLabel: 'Warranty disclaimer',
                    message: PCAT71_WARRANTY_DISCLAIMER,
                    visitHref: PCAT71_WARRANTY_URL,
    ...
    'e-expert': {
        card1: {
            ...
            description:
                `100% electric driving range up to 330km, with a 100kW motor and 75kWh<span class="pcat71-infotip-keep">battery${config.buildInfoTipHtml({
                    ariaLabel: 'Battery disclaimer',
                    message: PCAT71_BATTERY_WLTP_DISCLAIMER,
        ...
        card2: {
            title: 'Touchscreen',
            description:
                `10” touchscreen with Apple CarPlay® and Android Auto™ <span class="pcat71-infotip-keep">wireless connectivity.${config.buildInfoTipHtml({
                    ariaLabel: 'Wireless connectivity disclaimer',
                    message: PCAT71_WIRELESS_DISCLAIMER,
    ...
    'boxer-van': {
        ...
        card2: {
            title: 'Touchscreen',
            description:
                `9” touchscreen with Apple CarPlay® and Android Auto™ <span class="pcat71-infotip-keep">connectivity.${config.buildInfoTipHtml({
